...
static bool waitFriends(int id)
{
    ...
    sh->fSt.tableClients++;                                                               /* update number of clients */
    if(sh->fSt.tableClients == 1){                                                          /* first client */
        first = true;
        sh->fSt.st.clientStat[id] = WAIT_FOR_FRIENDS;  
        sh->fSt.tableFirst = id;                                                        // guardar o id do primeiro cliente
        saveState(nFic, &sh->fSt);                                                           
    }
    else if(sh->fSt.tableClients == TABLESIZE){                                                 /* last client */                                             // update internal state   
        ...
        sh->fSt.st.clientStat[id] = WAIT_FOR_FRIENDS;                                       /* update internal state */
        saveState(nFic, &sh->fSt);
    }
    ///
